...
const CandidateAddresses = require("./candidateAddresses.json");
...
contract("ElectionController",async (accounts) => {
    ...
    beforeEach( async () => {
        ...
        //electionController = await ElectionController.deployed();

        for (let i=0;i<districtsNo;i++){
            candidates.push([]);
            candidateAddresses.push([]);

            for(let j=0;j<Math.floor(CandidateAddresses[j].length/2);j++){
            ...
            for(let j=5;j<Math.ceil(CandidateAddresses[j].length/2);j++){
                let a = await web3.eth.accounts.create().address;
                candidateAddresses[i].push(a);
                candidates[i].push([
                    a,//await web3.eth.accounts.create(),//address
        ...
        let tx = await electionController.launchElection(_name,_voteStart,_voteEnd,numberOfPartiesContesting,candidates,{gasPrice: 140000000000,gas:7142857});
        console.log(tx);

...
contract("VoteToken", async accounts => {
  ...
  beforeEach(async () => {
    ...
    name = "TEST";
    voteToken = await VoteToken.new(name,"",voteStart,voteEnd,{from: accounts[1]});
  });
/*
  it("Should have the attributes passed to it", async ()=>{
    ...
    let _voteEnd = await voteToken.voteEnd.call({from: accounts[1]});
    console.log(_voteEnd);

    let _voteStart = await voteToken.voteStart.call({from: accounts[1]});
  ...
  it("Minting should increase balance. New token should have correct district associated with it. Should be impossible to mint new token to address that already has one.", async () => {
    ...
    await voteToken.mint(accounts[1],tokenID,district);
    

    let _balance = await voteToken.balanceOf(accounts[1]);
  ...
  it("Should revert all transactions when current time is after set voting period but allow owning address to mint tokens provided the end time of the election hasn't come to pass.", async () => {
    ...
    let _voteToken = await VoteToken.new(name,"",voteStart,voteEnd,{from: accounts[1]});
    await _voteToken.setAllowed(accounts[1]);
    await truffleAssert.reverts(_voteToken.mint(accounts[1],1,13),"Election is over.");
    //await truffleAssert.reverts(_voteToken.transferFrom(accounts[1],accounts[1],1),reason="Voting Not Open.","transferFrom does not revert outside of voting range");

    
    voteStart = (new Date(2050,12,30)).getTime();
    voteEnd = (new Date(2050,12,31)).getTime();
    _voteToken = await VoteToken.new(name,"",voteStart,voteEnd,{from: accounts[1]});
    //await _voteToken.setAllowed(accounts[1]);
...
*/
  it('Should Revert transfers from non-allowed addresses', async () => {
    ...
    await truffleAssert.reverts(_voteToken.transferFrom(accounts[1],accounts[1],1),reason="Calling Address not in list of allowed contracts.");
  });
/*
  ...
  , async () => {
    let _tokenId = 2;
    await voteToken.mint(accounts[1],_tokenId,13);
  ...
  it('Should not Revert transfers from allowed addresses.', async () => {
    ...
    await voteToken.mint(accounts[1],tokenID,districtNo);
